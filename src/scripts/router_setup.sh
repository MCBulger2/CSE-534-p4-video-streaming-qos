...
#complite
docker cp http2.p4 fabric_p4:/root/http2.p4
docker exec -w /root fabric_p4 sh -c 'p4c --target bmv2 --arch v1model http2.p4'
docker exec -d -it fabric_p4 sh -c 'simple_switch --log-console --interface 1@'${port1_iface}' --interface 2@'${port2_iface}' --interface 3@'${port3_iface}' /root/http2.json'
